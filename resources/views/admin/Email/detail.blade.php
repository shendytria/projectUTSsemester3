        ...
        <div class="main-panel">
            <div class="content-wrapper">
              <div class="card">
                ...
                <p class="card-text">Pengirim: {{ $email->sender }}</p>
                <p class="card-text">Tanggal: {{ \Carbon\Carbon::parse($email->create_date)->format('d M Y, H:i') }}</p>
                <hr>
                <p>{{ $email->message_text }}</p>
                </div>
            </div>
                <!-- Form untuk membalas email -->
                <div class="mt-4">
                    <h4>Balas Email</h4>
                    <form action="{{ route('email.reply', ['id' => $email->id_message]) }}" method="POST">
                        @csrf
